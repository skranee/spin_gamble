...
const dice_router = express.Router();

let games = [];

dice_router.post("/bet", auth(),  async (req, res) =>
{
    try
    {
        // TODO: process bets :)
        ...
        if(gameIndex < 0 || this_game.roblox_id != user.roblox_id || isNaN(req.body.bet_amount) || isNaN(req.body.game_bet) || req.body.game_bet < 2 || req.body.game_bet > 98){
            res.send("blocked");
            return
        }
        ...
        FairCal({serverseed:this_game.game_seed, clientseed:req.body.input_seed}, (result, seednohash, seedhash) => {
            game_result = result%101;
        }); 

        // calculate win/lose
        if(game_result > req.body.game_bet){
            win_amount = req.body.bet_amount * 100/(100-req.body.game_bet)*0.99;
        }
...
dice_router.post("/create_game", auth(),  async (req, res) =>
{
    try
    {
        // create game 
...
dice_router.post("/get_game", auth(),  async (req, res) =>
{
    try
    {
        // create game 
